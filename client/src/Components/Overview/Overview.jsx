  ...
  componentDidMount() {
    axios.get('/products/' + this.props.currentProduct + '/styles')
      .then((results) => {
        ...
        this.setState({ prevPhotoUrl: results.data.results[0].photos[results.data.results[0].photos.length - 1].url.replace(/"/g, '') })
        this.setState({ currentPhotoUrl: results.data.results[0].photos[0].url.replace(/"/g, '') })
        this.setState({ nextPhotoUrl: results.data.results[0].photos[1].url.replace(/"/g, '') })
